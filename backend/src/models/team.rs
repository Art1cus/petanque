...
impl Team {
    pub async fn all<C: GenericClient>(client: &C) -> Result<TeamList, Error> {
        let stmt = client.prepare("SELECT team_id, team_name, captain_name, group_id FROM teams").await?;
    ...
    pub async fn by_id<C: GenericClient>(client: &C, team_id: i32) -> Result<TeamList, Error> {
        let stmt = client.prepare("SELECT team_id, team_name, captain_name, group_id FROM teams WHERE team_id = $1").await?;
